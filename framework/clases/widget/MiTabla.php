	...
	class MiTabla extends CWidget
	{
		...
		public function __construct($cabecera,$filas,$atributosHTML=array())
		{
			...
			if (!isset($this->_atributosHTML["border"]))
				...
				//establezco la clase siempre la misma, para que lo ejecute bootstrap
				$this->_atributosHTML["class"]="table table-hover mb-0";
		...
		public function dibujaApertura()
		{
			...
			echo CHTML::dibujaEtiqueta("div",array("class"=>"card card-cascade narrower"),
										"",false);
			
			echo CHTML::dibujaEtiqueta("div", array("class"=>"px-4"),"",false);
			
			echo CHTML::dibujaEtiqueta("div", array("class"=>"table-wrapper"),"",false);
			...
			<thead class="red lighten-1 white-text">							
    			<tr>
    				...
    				foreach ($this->_columnas as $columna) 
    				{
    				  ...
    				  if ($columna["VISIBLE"])	
    				   		echo "<$etiqueta class='th-lg'>".$columna["ETIQUETA"]."</$eti>";	
    				}
